...
public class EntityHandler
{
	...
	{
		npcs = (NPCDef[]) PersistenceManager.load( new File( Config.CLIENT_DIR,
				"NPCs.rscd" ) );
		items = (ItemDef[]) PersistenceManager.load( new File( Config.CLIENT_DIR,
				"Items.rscd" ) );
		textures = (TextureDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Textures.rscd" ) );
		animations = (AnimationDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Animations.rscd" ) );
		spells = (SpellDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Spells.rscd" ) );
		prayers = (PrayerDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Prayers.rscd" ) );
		tiles = (TileDef[]) PersistenceManager.load( new File( Config.CLIENT_DIR,
				"Tiles.rscd" ) );
		doors = (DoorDef[]) PersistenceManager.load( new File( Config.CLIENT_DIR,
				"Doors.rscd" ) );
		elevation = (ElevationDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Elevation.rscd" ) );
		objects = (GameObjectDef[]) PersistenceManager.load( new File(
				Config.CLIENT_DIR, "Objects.rscd" ) );
