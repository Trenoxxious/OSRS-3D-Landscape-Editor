...
public final class mudclient extends GameWindowMiddleMan {
    ...
    /*
     * editmode, determines what happens on mouse click 0 = view only - nothing happens 1 = land
     * editor
     */
    ...
    private boolean heightTargetMode = false; // Use target height instead of raise/lower
    private int targetHeight = 10; // Target height for terrain
    ...
    private static final long PAINT_INTERVAL = 100; // 0.1 seconds in milliseconds
    ...
    public void reloadSection(String sectionName) // update edit siin pets
    {
        ...
        try {
            h = wildYSubtract = Integer.parseInt(sectionName.substring(hIndex + 1, xIndex));
            x = Integer.parseInt(sectionName.substring(xIndex + 1, yIndex));
            y = Integer.parseInt(sectionName.substring(yIndex + 1, sectionName.length()));
        } catch (Exception e) {
            return;
        }
    ...
    public static final void main(String[] args) throws Exception {
        ...
        mc.setLogo(Toolkit.getDefaultToolkit()
                .getImage(Config.CLIENT_DIR + File.separator + "Loading.rscd"));
    ...
    private void calculateMousePosition(int x, int y) {
        ...
        for (int j1 = 0; j1 < i1; j1++) {
            ...
            // Heights are y-shifted on the server to determine a unique y coord for different
            // heights
            int worldYPosBeforeHeightYShift = ((sectionY - 38) * 48) + sectionYPos;
            worldYPos = (sectionH * Config.HEIGHT_Y_SHIFT)
                    + (worldYPosBeforeHeightYShift % Config.HEIGHT_Y_SHIFT);
    ...
    protected void handleMouseUp(int button, int x, int y) {
        System.out.println("Mouse up: button=" + button + ", x=" + x + ", y=" + y + ", editMode="
                + editMode + ", activeTool=" + activeTool + ", sampleMode=" + sampleMode);

        // Reset mouse pressed state for paint mode
        if (button == 1) {
        ...
        // Handle sample mode clicking first (works for any button when sample mode is active)
        if (sampleMode) {
            calculateMousePosition(x, y);
            sampleTileProperties(sectionXPos, sectionYPos);
            sampleMode = false; // Auto-disable after sampling
            return; // Exit early - sample mode only samples, doesn't edit
        }
        ...
        if (button == 2) {
            ...
            if (actuallyDraggedRight) {
                ...
                if (heightTargetMode) {
                    // Target mode - both buttons work toward target
                    System.out.println("Height tool: targeting " + targetHeight + " at "
                            + sectionXPos + ", " + sectionYPos);
                    paintHeight(sectionXPos, sectionYPos, true); // Parameter ignored in target mode
                } else {
                    // Normal mode - right click lowers
                    System.out.println(
                            "Height tool: lowering at " + sectionXPos + ", " + sectionYPos);
            ...
            if (actuallyDraggedMiddle) {
                ...
                actuallyDraggedMiddle = false;
                System.out.println(
                        "Middle drag ended - no tool action (middle mouse never edits terrain)");
        ...
        if (button == 1) {
            ...
            // Height tool terrain editing
                ...
                if (heightTargetMode) {
                    ...
                    System.out
                            .println("Height tool: raising at " + sectionXPos + ", " + sectionYPos);
    ...
    protected final void handleMouseDown(int button, int x, int y) {
        ...
        // Set mouse pressed state for paint mode
        if (button == 1) {
            leftMousePressed = true;
            lastPaintTime = 0; // Reset paint timer
        } else if (button == 2) {
        ...
        /*
         * calculateMousePosition( x, y ); editElevation( worldXPos, worldYPos ); if( button == 1 &&
         * !mDragStart ) { mDragStartMousePosX = x; mDragStartMousePosY = y;
         * 
         * mDragCurrentMousePosX = x; mDragCurrentMousePosY = y;
         * 
         * // int oldPosX = ourPlayer.currentX; // int oldPosY = ourPlayer.currentY;
         * 
         * // ourPlayer.currentX = ourPlayer.waypointsX[0] = worldXPos; // ourPlayer.currentY =
         * ourPlayer.waypointsY[0] = worldYPos;
         * 
         * calculateMousePosition( x, y );
         * 
         * // ourPlayer.currentX = ourPlayer.waypointsX[0] = oldPosX; // ourPlayer.currentY =
         * ourPlayer.waypointsY[0] = oldPosY;
         * 
         * dragStartPosX = worldXPos; dragStartPosY = worldYPos;
         * 
         * mDragStart = true; }
         */
    }
    ...
    protected final void handleMouseDrag(int button, int x, int y) {
        ...
        // In paint mode, handle continuous painting instead of camera controls (unless SPACE is
        // held)
        if (paintMode && activeTool > 0 && !spacePressed
                && ((button == 1 && leftMousePressed) || (button == 2 && rightMousePressed))) {
            long currentTime = System.currentTimeMillis();
            if (currentTime - lastPaintTime >= PAINT_INTERVAL) {
                // Perform continuous painting based on active tool and button
                if (activeTool == 1) {
                    // Height tool: in target mode, both buttons work toward target
                    if (heightTargetMode) {
                        paintHeight(sectionXPos, sectionYPos, true); // Parameter ignored in target
                                                                     // mode
                    } else {
                        // Normal mode: left=raise, right=lower
                        paintHeight(sectionXPos, sectionYPos, button == 1);
                    }
                } else if (activeTool == 2) {
                    // Texture tool: both buttons paint texture
                    paintTexture(sectionXPos, sectionYPos);
                } else if (activeTool == 3) {
                    // Overlay tool: both buttons paint overlay
                    paintOverlay(sectionXPos, sectionYPos);
                }
                lastPaintTime = currentTime;
            }
            return; // Skip camera controls when paint mode is active
        }

        // Camera controls (normal drag behavior or when SPACE is held in paint mode)
    ...
    private final void drawGame() {
        ...
        /*
         * if( now - lastFrame > (1000 / Config.MOVIE_FPS) && recording) { try { lastFrame = now;
         * frames.add( getImage() ); } catch( Exception e ) { } }
         */
        ...
        /*
         * for (int i = 0; i < 64; i++) {
         * gameCamera.removeModel(engineHandle.aModelArrayArray598[lastWildYSubtract][i]); if
         * (lastWildYSubtract == 0) {
         ...
         * //gameCamera.removeModel(engineHandle.aModelArrayArray598[2][i]); } if (lastWildYSubtract
         * == 0 && (engineHandle.walkableValue[ourPlayer.currentX / 128][ourPlayer.currentY / 128] &
         * 0x80) == 0) { if (showRoof) {
         * gameCamera.addModel(engineHandle.aModelArrayArray598[lastWildYSubtract][i]); if
         * (lastWildYSubtract == 0) { //gameCamera.addModel(engineHandle.aModelArrayArray580[1][i]);
        ...
        int cameraY =
                (activeTool == 1) ? fixedCameraHeight : -engineHandle.getAveragedElevation(l5, i8);
        ...
        if (mDragStart) {
            ...
            minX = Math.min(gameGraphics.getImageX() + gameGraphics.getImageWidth(), minX);

            maxX = Math.max(gameGraphics.getImageX(), maxX);
            maxX = Math.min(gameGraphics.getImageX() + gameGraphics.getImageWidth(), maxX);

            minY = Math.max(gameGraphics.getImageY(), minY);
            minY = Math.min(gameGraphics.getImageY() + gameGraphics.getImageHeight(), minY);

            maxY = Math.max(gameGraphics.getImageY(), maxY);
            maxY = Math.min(gameGraphics.getImageY() + gameGraphics.getImageHeight(), maxY);

            if (maxX > minX && maxY > minY) {
                gameGraphics.drawBoxEdge(minX, minY,
                        Math.min(maxX - minX, gameGraphics.getImageWidth() - minX),
                        Math.min(maxY - minY, gameGraphics.getImageHeight() - minY), 0xffffff);
            }
        ...
        if (paintMode) {
            int rightAlign = gameGraphics.getImageWidth() - 250;
            gameGraphics.drawString("@gre@PAINT MODE (P) - Hold SPACE for camera", rightAlign, 100,
                    0, 0);
        }
        ...
        gameGraphics.drawString((activeTool == 1 ? "@gre@" : "@whi@") + "Height Tool (H)", 25, 145,
                0, 0);

        int textureToolColor = (activeTool == 2) ? 0x00ff00 : 0xffffff;
        gameGraphics.drawBoxEdge(20, 155, 170, 20, textureToolColor);
        gameGraphics.drawString((activeTool == 2 ? "@gre@" : "@whi@") + "Texture Tool (J)", 25, 170,
                0, 0);

        int overlayToolColor = (activeTool == 3) ? 0x00ff00 : 0xffffff;
        gameGraphics.drawBoxEdge(20, 180, 170, 20, overlayToolColor);
        gameGraphics.drawString((activeTool == 3 ? "@gre@" : "@whi@") + "Overlay Tool (K)", 25, 195,
                0, 0);

        // Tool-specific settings
        if (activeTool == 1) {
            gameGraphics.drawString("@whi@Radius: " + heightToolRadius + " ([/])", 25, 215, 0, 0);
            gameGraphics.drawString("@whi@Strength: " + heightToolStrength + " (T/G)", 25, 225, 0,
                    0);
            gameGraphics.drawString(
                    "@whi@Softness: " + String.format("%.1f", heightToolSoftness) + " (Y/U)", 25,
                    235, 0, 0);

            if (heightTargetMode) {
                gameGraphics.drawString("@gre@Target Mode: " + targetHeight + " (V/C)", 25, 245, 0,
                        0);
                gameGraphics.drawString("@gre@Click: Paint to Target (R=Toggle)", 25, 255, 0, 0);
            } else {
                gameGraphics.drawString("@whi@L-Click: Raise, R-Click: Lower", 25, 245, 0, 0);
                gameGraphics.drawString("@whi@R: Toggle Target Mode", 25, 255, 0, 0);
            }
        } else if (activeTool == 2) {
            gameGraphics.drawString("@whi@Radius: " + textureToolRadius + " ([/])", 25, 215, 0, 0);
            gameGraphics.drawString(
                    "@whi@Softness: " + String.format("%.1f", textureToolSoftness) + " (Y/U)", 25,
                    225, 0, 0);
        ...
        gameGraphics.drawString("@whi@S - Toggle Shadows " + (shadowsEnabled ? "ON" : "OFF"),
                rightAlign, 90, 0, 0);
    ...
    protected final void startGame() {
        ...
        gameGraphics = new GameImageMiddleMan(DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT + 12 + 99,
                4000, this);
        gameGraphics._mudclient = this;
        gameGraphics.setDimensions(0, 0, DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT + 12);
        ...
        gameCamera.setCameraSize(DEFAULT_WINDOW_WIDTH / 2, DEFAULT_WINDOW_HEIGHT / 2,
                DEFAULT_WINDOW_WIDTH / 2, DEFAULT_WINDOW_HEIGHT / 2, DEFAULT_WINDOW_WIDTH,
                cameraSizeInt);
    ...
    private void sampleTileProperties(int x, int y) {
        if (!sectionLoaded) {
            System.out.println("Cannot sample tile: section not loaded");
            return;
        }

        Tile tile = getTileAtPos(x, y);
        if (tile == null) {
            System.out.println("Cannot sample tile at coordinates: " + x + ":" + y);
            return;
        }

        // Sample properties based on active tool
        if (activeTool == 1) // Height tool
        {
            int sampledHeight = tile.groundElevation & 0xFF; // Convert byte to unsigned int
            targetHeight = sampledHeight;
            heightTargetMode = true; // Auto-enable target mode when sampling
            System.out.println("Sampled height: " + sampledHeight + " (target mode enabled)");
        } else if (activeTool == 2) // Texture tool
        {
            int sampledTexture = tile.groundTexture & 0xFF;
            selectedTexture = sampledTexture;
            System.out.println("Sampled texture: " + sampledTexture);
        } else if (activeTool == 3) // Overlay tool
        {
            int sampledOverlay = tile.groundOverlay & 0xFF;
            selectedOverlay = sampledOverlay;
            System.out.println("Sampled overlay: " + sampledOverlay);
        } else {
            // No tool active - sample all properties and display info
            int height = tile.groundElevation & 0xFF;
            int texture = tile.groundTexture & 0xFF;
            int overlay = tile.groundOverlay & 0xFF;
            System.out.println("Tile properties - Height: " + height + ", Texture: " + texture
                    + ", Overlay: " + overlay);
        }
    }
            ...
            for (int i1 = 0; i1 < ai1.length; i1++) {
                ...
                if (j1 > temp[255]) {
                    for (int k1 = 1; k1 < 256; k1++) {
                        ...
                        dictionary[k1] = ((i1 & 0x7c00) << 9) + ((i1 & 0x3e0) << 6)
                                + ((i1 & 0x1f) << 3) + 0x40404;
                        temp[k1] = j1;
                        break;
                    }
    ...
    protected final byte[] load(String filename) {
        return super.load(Config.CLIENT_DIR + File.separator + "data" + File.separator + filename);
    }
    ...
    private final void processGame() {
        ...
        if (cameraAutoAngleDebug) {
            ...
            /*
             * if( lastAutoCameraRotatePlayerX - ourPlayer.currentX < -500 ||
             * lastAutoCameraRotatePlayerX - ourPlayer.currentX > 500 || lastAutoCameraRotatePlayerY
             * - ourPlayer.currentY < -500 || lastAutoCameraRotatePlayerY - ourPlayer.currentY > 500
             * ) { lastAutoCameraRotatePlayerX = ourPlayer.currentX; lastAutoCameraRotatePlayerY =
             * ourPlayer.currentY; }
             * 
             * if( lastAutoCameraRotatePlayerX != ourPlayer.currentX ) { int scaleMovementDivisor =
             * (16 + (cameraHeight - 500) / 15); if(scaleMovementDivisor < 1) //cannot divide by
             * zero, negative numbers cause camera to spaz scaleMovementDivisor = 1;
             * lastAutoCameraRotatePlayerX += (ourPlayer.currentX - lastAutoCameraRotatePlayerX) /
             * scaleMovementDivisor; } if( lastAutoCameraRotatePlayerY != ourPlayer.currentY ) { int
             * scaleMovementDivisor = (16 + (cameraHeight - 500) / 15); if(scaleMovementDivisor < 1)
             * //cannot divide by zero, negative numbers cause camera to spaz scaleMovementDivisor =
             * 1; lastAutoCameraRotatePlayerY += (ourPlayer.currentY - lastAutoCameraRotatePlayerY)
             * / scaleMovementDivisor; }
             */
        }
    ...
    /**
     * Converts a byte angle to degrees. Byte angles larger than 255 will be converted into lowest
     * represntation of degrees
     * 
    ...
    private void addUndoAction(UndoAction action) {
        ...
        System.out.println(
                "Added undo action: " + action.actionType + " at " + action.x + "," + action.y);
    }
    ...
    private void performUndo() {
        ...
        System.out.println(
                "Undoing " + action.actionType + " action at " + action.x + "," + action.y);
    ...
    private void handleToolShortcuts() {
        // Sample mode toggle with 'I' key (for "Inspect")
        if (super.keyDown == 'i' || super.keyDown == 'I') {
            sampleMode = !sampleMode;
            System.out.println("Sample mode "
                    + (sampleMode ? "ON - Click any tile to sample properties" : "OFF"));
        ...
        if (activeTool > 0) {
            ...
            Cursor invisibleCursor =
                    getToolkit().createCustomCursor(image, new Point(0, 0), "invisible");
        ...
        if (activeTool == 1) // Height tool controls
        {
            if (super.keyDown == ']') {
                ...
                super.keyDown = 0;
            } else if (super.keyDown == 'r' || super.keyDown == 'R') {
                heightTargetMode = !heightTargetMode;
                System.out.println("Height target mode " + (heightTargetMode ? "ON" : "OFF"));
                super.keyDown = 0;
            } else if (super.keyDown == 'v' || super.keyDown == 'V') {
                targetHeight = Math.min(255, targetHeight + 1);
                super.keyDown = 0;
            } else if (super.keyDown == 'c' || super.keyDown == 'C') {
        ...
        } else if (super.keyDown == 26) // Ctrl+Z
        {
            performUndo();
            super.keyDown = 0;
        } else if (super.keyDown == 'p' || super.keyDown == 'P') {
            if (editMode == 1) // Only allow paint mode in edit mode
            {
                paintMode = !paintMode;
                System.out.println("Paint mode " + (paintMode ? "ON" : "OFF"));
            }
            super.keyDown = 0;
        }

        // Track space key for paint mode override
        spacePressed = (super.keyDown == 32);
    }
    ...
    private void paintHeight(int centerX, int centerY, boolean raise) {
        ...
        System.out.println("Height tool: " + (raise ? "raising" : "lowering") + " at " + centerX
                + ", " + centerY);
        ...
        for (int dx = -heightToolRadius; dx <= heightToolRadius; dx++) {
            for (int dy = -heightToolRadius; dy <= heightToolRadius; dy++) {
                ...
                Tile tile = getTileAtPos(x, y);
                if (tile != null) {
                    int currentElevation = tile.groundElevation & 0xFF; // Convert byte to unsigned
                                                                        // int
                    int newElevation;

                    if (heightTargetMode) {
                        // Target height mode: move toward target height
                        int heightDiff = targetHeight - currentElevation;
                        int change = (int) (heightDiff * (heightToolStrength / 20.0f) * falloff);
                        newElevation = currentElevation + change;
                    } else {
                        // Normal raise/lower mode
                        newElevation = currentElevation + (int) (strength * falloff);
                    }

                    tile.groundElevation = (byte) Math.max(0, Math.min(255, newElevation));
                    System.out.println("Tile at " + x + "," + y + ": " + currentElevation + " -> "
    ...
    private void paintTexture(int centerX, int centerY) {
        ...
        System.out.println("Texture tool: painting texture " + selectedTexture + " at " + centerX
                + ", " + centerY);
                    ...
                    if (distance <= textureToolRadius) {
                        ...
                        if (tile != null) {
                            ...
                            if (falloff > 0.9f) {
                                ...
                                int blendedTexture = (int) (currentTexture * (1.0f - falloff)
                                        + selectedTexture * falloff);
                            ...
                            if (tile.groundTexture != originalTile.groundTexture) {
                                // Store undo action
                                UndoAction action =
                                        new UndoAction(x, y, originalTile, tile, "texture");
    ...
    private void paintOverlay(int centerX, int centerY) {
        ...
        System.out.println("Overlay tool: painting overlay " + selectedOverlay + " at " + centerX
                + ", " + centerY);
    ...
    private void drawToolPreview() {
        ...
        if (activeTool == 1) {
            ...
            gameGraphics.drawString("@yel@I: Sample Mode " + (sampleMode ? "ON" : "OFF"), 25, 265,
                    0, 0);
        } else if (activeTool == 2) {
            gameGraphics.drawString("@whi@Radius: " + textureToolRadius + " ([/])", 25, 215, 0, 0);
            gameGraphics.drawString(
        ...
        if (sampleMode && activeTool == 0) {
            gameGraphics.drawString("@gre@SAMPLE MODE ACTIVE", 25, 215, 0, 0);
            gameGraphics.drawString("@gre@Click any tile to inspect", 25, 225, 0, 0);
            gameGraphics.drawString("@yel@I: Sample Mode OFF", 25, 235, 0, 0);
        }
    ...
    /**
     * Move the loaded section in a cardinal direction; 0 - North, 1 - East, 2 - South, 3 - West, 4
     * - Up, 5 - Down
     * 
